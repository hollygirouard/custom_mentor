...
const SCREENS = [
  {
    ...
    title: 'Search for Mentor/Mentee',
    route: '/search',
...
export default class App extends Component {
  checkAuth(ComponentToRender) {
    return props => (this.props.isLoggedIn
  ...
  renderRoutes() {
    return (
      <div>
        {SCREENS.map(screen => (<Route path={screen.route} key={screen.route} exact render={this.checkAuth(screen.component)}/>))}
      </div>
    );
  }
  ...
  renderNav() {
    return (
      <nav className="navbar navbar-default navbar-fixed-top" id={this.props.isLoggedIn
        ? 'member_sidebar'
        : 'documenter_sidebar'}>
      ...
      <footer id="footer" className="footer">
        <div className="container text-center">
          ...
          <div className="credits">
            ...
            Designed by
            <a href="https://bootstrapmade.com/" target="blank">BootstrapMade.com</a>
          </div>
        </div>
      </footer>
    );
    ...
    return this.props.isLoggedIn
      ? (
        <ul className="nav navbar-nav navbar-right">
          ...
          {SCREENS.filter(item => item.mainMenu).map(screen => (
          ...
          <li>
            <a color="primary" className="signout" onClick={() => this.props.userSignOut()}>Sign out</a>
          </li>
      ...
      : (
        <ul className="nav navbar-nav navbar-right">
          <a href="javascript:void(0)" class="closebtn" onClick={() => this.closeNav()}>&times;</a>
          <li>
            <a onClick={() => this.closeNav()} href="#signin">Sign-In/Sign-Up</a>
          </li>
          <li>
            <a onClick={() => this.closeNav()} href="#menteeInfo">Find a Mentor</a>
          </li>
          <li>
            <a onClick={() => this.closeNav()} href="#mentorInfo">Become a Mentor</a>
          </li>
          <li>
  ...
  render() {
    return (
      <Router>
        <div>
          {this.renderNav()}
          {< div className = "home" > <Route path="/" exact component={HomeContainer}/>
          {
            this.renderRoutes()
          }
